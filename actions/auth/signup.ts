...
import { fetchApiClient } from '@/lib/one_entry';
...
export const getSignupFormData = async (): Promise<IAttributes[]> => {
  try {
    ...
    if (!response?.attributes) {
      throw new Error('No form attributes found');
    }
    
    return response.attributes as unknown as IAttributes[];
  } catch (error: any) {
    console.error('Signup form data error:', error);
    throw new Error(`Fetching form data failed: ${error.message || 'Unknown error'}`);
  }
};
